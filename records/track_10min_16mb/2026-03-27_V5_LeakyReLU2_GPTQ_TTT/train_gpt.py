...
class Hyperparameters:
    ...
    xsa_last_n = int(os.environ.get("XSA_LAST_N", 4))  # XSA on last 4 layers (matches SOTA step time)
    xsa_skip_layers = os.environ.get("XSA_SKIP_LAYERS", "9,10")  # disable XSA on these during TTT
...
def _make_lora_fwd(mod, lA, lB, ow):
    def fwd(x):
        w = ow.to(x.dtype) + (lB @ lA).to(x.dtype)
...
def eval_val_sliding_ttt(
    ...
    xsa_st = {i: b.attn.use_xsa for i, b in enumerate(base_model.blocks)}
    for i, b in enumerate(base_model.blocks):
        if i in xsa_skip: b.attn.use_xsa = False
    base_model.eval(); lr, temperature = args.ttt_lora_rank, args.ttt_temperature
    targets = []
    for i, blk in enumerate(base_model.blocks):
        if i in xsa_skip: targets.append((f"b.{i}.v", blk.attn.c_v))
        targets.append((f"b.{i}.q", blk.attn.c_q))
    ow = {n: m.weight.data.detach().clone() for n, m in targets}
    lA, lB = {}, {}
    for n, m in targets:
        od, ind = m.weight.shape
        lA[n] = torch.zeros(lr, ind, device=device, dtype=torch.float32, requires_grad=True)
        lB[n] = torch.zeros(od, lr, device=device, dtype=torch.float32, requires_grad=True)
    vb = torch.zeros(args.vocab_size, device=device, dtype=torch.float32) if args.ogd_bias_enabled else None
    t0 = time.perf_counter()
    for wi, ws in enumerate(my_wins):
        if time.perf_counter() - t0 > args.ttt_time_budget:
            for i, b in enumerate(base_model.blocks): b.attn.use_xsa = xsa_st[i]
            rem = my_wins[wi:]
            cfn = torch.compile(base_model.forward_logits, dynamic=False, fullgraph=True) if _FA3 else base_model.forward_logits
            with torch.inference_mode():
                for bi in range(0, len(rem), 32):
                    bws = rem[bi:bi+32]; bsz = len(bws)
                    xb = torch.zeros(bsz, seq_len, dtype=torch.int64, device=device)
                    yb = torch.zeros(bsz, seq_len, dtype=torch.int64, device=device); wls = []
                    ...
                    with torch.autocast(device_type="cuda", dtype=torch.bfloat16): lg = cfn(xb)
                    if vb is not None: lg = lg + vb[None, None, :]
                    ...
                    for j, w in enumerate(bws):
                        ...
                        loss_sum += nll[j, sb:wl].to(torch.float64).sum(); tok_cnt += float(wl - sb)
        ...
        sf = {}
        for n, m in targets: sf[n] = m.forward; m.forward = _make_lora_fwd(m, lA[n], lB[n], ow[n])
        with torch.no_grad():
            with torch.autocast(device_type="cuda", dtype=torch.bfloat16): logits = base_model.forward_logits(x_win)
            logits = logits / temperature
            if vb is not None: logits = logits + vb[None, None, :]
            nll = F.cross_entropy(logits[0].float(), y_win[0], reduction="none")
            loss_sum += nll[s:wlen].to(torch.float64).sum(); tok_cnt += float(wlen - s)
        ...
        ap = [p for n, _ in targets for p in (lA[n], lB[n])]
        opt = torch.optim.AdamW(ap, lr=args.ttt_lr, weight_decay=0.01)
        for ep in range(args.ttt_epochs):
            for pg in opt.param_groups: pg['lr'] = args.ttt_lr * 0.5 * (1.0 + math.cos(math.pi * ep / args.ttt_epochs))
            opt.zero_grad()
            with torch.autocast(device_type="cuda", dtype=torch.bfloat16): al = base_model.forward_logits(x_win[:, :wlen])
            F.cross_entropy(al.reshape(-1, al.size(-1)).float(), y_win[:, :wlen].reshape(-1), reduction="mean").backward()
            opt.step()
        for n, m in targets: m.forward = sf[n]
        if wi + 1 < len(my_wins):
            nxt = my_wins[wi + 1]
            with torch.no_grad():
                if nxt < end:
                    for n, _ in targets: lA[n].mul_(args.ttt_warmstart_decay); lB[n].mul_(args.ttt_warmstart_decay)
                else:
                    for n, _ in targets: lA[n].zero_(); lB[n].zero_()
        if log_fn and (wi + 1) % 500 == 0:
            ibpb = (loss_sum / tok_cnt).item() / math.log(2.0) * (tok_cnt / byte_cnt).item()
            log_fn(f"ttt:window {wi+1}/{len(my_wins)} bpb:{ibpb:.4f} t:{time.perf_counter()-t0:.0f}s")
    for i, b in enumerate(base_model.blocks): b.attn.use_xsa = xsa_st[i]
    if dist.is_available() and dist.is_initialized():
        dist.all_reduce(loss_sum, op=dist.ReduceOp.SUM); dist.all_reduce(tok_cnt, op=dist.ReduceOp.SUM); dist.all_reduce(byte_cnt, op=dist.ReduceOp.SUM)
...
def quantize_int6_per_row(t: Tensor, clip_range: int = 31) -> tuple[Tensor, Tensor]:
    ...
    return q, scale
def quantize_int6_gptq(weight: Tensor, hessian: Tensor, clip_range: int = 31, block_size: int = 128) -> tuple[Tensor, Tensor]:
    ...
    for pct in [0.9990, 0.9995, 0.9999, 0.99999, 1.0]:
        Wt = W.clone()
        row_clip = torch.quantile(Wt.abs(), pct, dim=1) if pct < 1.0 else Wt.abs().amax(dim=1)
        s = (row_clip / clip_range).clamp_min(1.0 / clip_range).to(torch.float16)
        Qt = torch.zeros_like(Wt, dtype=torch.int8)
        ...
        mse = (W - Qt.float() * s.float().unsqueeze(1)).pow(2).mean().item()
        if mse < best_err: best_q, best_s, best_err = Qt, s, mse
    return best_q[:, torch.argsort(perm)].contiguous(), best_s
def selective_prune_to_fit(quant_result, quant_meta, code_bytes, target_bytes=15_900_000):
    ones_info = []
...
def main() -> None:
    ...
    log0(f"seed:{args.seed} ttt:{args.ttt_enabled} ogd:{args.ogd_bias_enabled}")
    ...
    code_bytes = len(code.encode("utf-8"))
    # Selective pruning to fit under 16MB (PR #609)
    log0("selective_prune:binary search to fit under 16MB...")
    selective_prune_to_fit(quant_result, quant_meta, code_bytes, target_bytes=15_900_000)
    ...
    if args.ttt_enabled:
        ...
        torch.cuda.synchronize()
        log0(f"final_ttt val_loss:{ttt_loss:.4f} val_bpb:{ttt_bpb:.4f} eval_time:{1000.0*(time.perf_counter()-t_ttt):.0f}ms")
