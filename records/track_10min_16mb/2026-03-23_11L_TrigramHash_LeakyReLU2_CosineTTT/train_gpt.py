...
def eval_val_sliding_ttt(
    ...
    """Sliding window eval with backward-looking TTT via LoRA on MLP projections.
    For each window, adapt the model on the prefix (already-scored tokens) using LoRA,
    then score the new tokens. This is legal because we only train on backward context."""
    seq_len = eval_seq_len or args.train_seq_len
    total_tokens = val_tokens.numel() - 1
    window_starts = [ws for ws in range(0, total_tokens, stride)
                     if min(ws + seq_len, total_tokens) - ws >= 1]
    ...
    byte_count = torch.zeros((), device=device, dtype=torch.float64)
    base_model.eval()
    # Identify MLP projection layers for LoRA
    lora_targets = []
    ...
    for wi, ws in enumerate(my_windows):
        ...
        if elapsed > max_eval_time:
            if log_fn:
                log_fn(f"ttt:time_budget_exceeded at window {wi}/{len(my_windows)}, falling back to no-TTT")
            # Fall back to non-TTT for remaining windows
            break
        end = min(ws + seq_len, total_tokens)
        wlen = end - ws
        chunk = val_tokens[ws:end + 1].to(dtype=torch.int64, device=device)
        x_win = chunk[:-1].unsqueeze(0)  # [1, wlen]
        y_win = chunk[1:].unsqueeze(0)   # [1, wlen]
        # Pad to seq_len
        if wlen < seq_len:
            ...
            x_padded = x_win
            y_padded = y_win
        # Determine scored region
        s = 0 if ws == 0 else max(wlen - stride, 0)
        prefix_len = s  # tokens 0..s-1 are already scored (backward context)
        # TTT: adapt on prefix if we have enough context
        if prefix_len >= 16 and ttt_epochs > 0:
            # Create LoRA parameters for MLP layers
            lora_A = {}
            lora_B = {}
            lora_params = []
            for name, module in lora_targets:
            ...
            optimizer = torch.optim.AdamW(lora_params, lr=ttt_lr, weight_decay=ttt_wd, betas=(0.9, 0.999))
            # Prefix data for TTT training
            x_prefix = x_padded[:, :prefix_len]
            y_prefix = y_padded[:, :prefix_len]
            total_ttt_steps = ttt_epochs
            for epoch in range(total_ttt_steps):
                # Cosine schedule
                cos_lr = ttt_lr * 0.5 * (1.0 + math.cos(math.pi * epoch / total_ttt_steps))
                for pg in optimizer.param_groups:
                    pg['lr'] = cos_lr
                # Apply LoRA deltas to weights
                for name, module in lora_targets:
                    delta = (lora_B[name] @ lora_A[name]).to(module.weight.dtype)
                    module.weight.data = original_weights[name] + delta
                optimizer.zero_grad()
                with torch.autocast(device_type="cuda", dtype=torch.bfloat16):
                    loss = base_model(x_prefix, y_prefix)
                loss.backward()
                optimizer.step()
            # Apply final LoRA deltas for scoring
            with torch.no_grad():
                for name, module in lora_targets:
                    delta = (lora_B[name] @ lora_A[name]).to(module.weight.dtype)
                    module.weight.data = original_weights[name] + delta
    ...
    # Handle remaining windows without TTT if time budget exceeded
...
def main() -> None:
    ...
    log0(f"ttt:enabled={args.ttt_enabled} rank={args.ttt_lora_rank} lr={args.ttt_lr} epochs={args.ttt_epochs}")
    log0(
